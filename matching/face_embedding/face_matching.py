...
class Tracklet():
    ...
    def delete_tail_track(self):
        ...
        while get_score(self.probe[-1]) < 0:
            ...
            if str(self.id) in self.batch_tracks[frameid] and \
                ...
                logger.debug("early stop in delete_tail_track for track {} with id {}".format(self.get_trackid(), self.id))
    ...
    def compare_motion(self, other):
        ...
        if self.get_nof_detections() < 2 or other.get_nof_detections() < 2 or min(np.abs(self.get_start_detection() - other.get_stop_detection()), np.abs(self.get_stop_detection() - other.get_start_detection())) > 10:
            return 0
    ...
    def compare(self, other):
        ...
        logger.debug('motion affinity between {} and {} is: {}'.format(self.get_trackid(), other.get_trackid(), motion_affinity))
        logger.debug('appearance affinity between {} and {} is: {}'.format(self.get_trackid(), other.get_trackid(), appearance_affinity))
...
class Identity():
    ...
    def compare(self, other):
        ...
        appearance_affinity = self.compare_appearance(other)
        motion_affinity = self.compare_motion(other)
        logger.debug('motion affinity between {} and {} is: {}'.format(self.get_trackids(), other.get_trackids(), motion_affinity))
        logger.debug('appearance affinity between {} and {} is: {} with {} and {}'.format(self.get_trackids(), other.get_trackids(), appearance_affinity, len(self.get_features_list()), len(other.get_features_list())))
            ...
            for i, probe in enumerate(probes):
                if tent_tl.get_trackid() == probe.get_trackid():
                    ...
                    if tent_tl.get_nof_detections() + probe.get_nof_detections() >= self.min_detections_track:
                        logger.debug('confirm tentative tracklet {} to probe {}'.format(tent_tl.get_trackid(), probe.get_trackid()))
                        ...
                        #discard both
                        logger.debug('discard tentative tracklet {} and probe {}'.format(tent_tl.get_trackid(), probe.get_trackid()))
        ...
        for probe in probes:
            ...
            if probe.get_nof_detections() < self.min_detections_batch:
                if probe.get_stop_track() != probe.fids[-1]:
                    ...
                    logger.debug('short tracklet due to interuption, add {} to tentative tracklets'.format(probe.get_trackid()))
...
#                    probes.remove(probe)
        ...
        logger.debug('add {} confirmed and assigned probes for reference in next batch'.format(len(self.confirmed_tracklets)))
    ...
    def new_match_in_batch_hungarian(self, probes, threshold = 0.85):
        ...
        for i, row_ind in enumerate(row_inds):
            if cost_matrix[row_ind][col_inds[i]] < threshold:
                logger.debug('match in batch: {} and {} with {}'.format(probes[row_ind].get_trackid(), probes[col_inds[i]].get_trackid(),
        ...
        for id, probe in enumerate(self.identities_in_batch):
            if not mark_p[id]:
                ...
                if not (cost_matrix[id] > threshold).all():
                    ...
                    logger.debug('have potential for match with {}'.format(cost_matching[np.where(cost_matching < threshold)[0]]))
