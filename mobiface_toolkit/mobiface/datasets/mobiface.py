...
class MOBIFACE(object):
    def __init__(self, root_dir, subset='all'):
        assert subset in ['all', 'train', 'test', 'vtb']
        ...
        self.vtb_meta_fn = osp.join(root_dir, 'vtb.meta.csv')
        ...
        self.vtb_meta = pd.read_csv(self.vtb_meta_fn, index_col=0).transpose().to_dict()
        ...
        if subset == 'all':
            self.meta = {**self.train_meta, **self.test_meta, **self.vtb_meta}  # In Python 3.5 or greater
        elif subset == 'train':
            self.meta = self.train_meta
        elif subset == 'vtb':
            self.meta = self.vtb_meta
        else:
        ...
        for k, v in self.meta.items():
            ...
            elif k in self.vtb_meta.keys():
                self.anno_files.append(osp.abspath(osp.join(self.root_dir, 'vtb', k + '.annot.csv')))
            else:
