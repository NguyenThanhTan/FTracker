...
from ..datasets import MOBIFACE
from ..utils.metrics import rect_iou, center_error, norm_center_error
from ..utils.viz import show_frame, draw_frame
from sklearn.metrics import auc


class ExperimentMobiFace(object):
    r"""Experiment pipeline and evaluation toolkit for MobiFace dataset.
    
    Args:
        root_dir (string): Root directory of MobiFace dataset.
        subset (string, optional): Subset of MobiFace dataset.
        result_dir (string, optional): Directory for storing tracking
            results. Default is ``./results``.
        report_dir (string, optional): Directory for storing performance
            evaluation results. Default is ``./reports``.
    """
    def __init__(self, root_dir, subset='all', result_dir='results', report_dir='reports',):
        super(ExperimentMobiFace, self).__init__()
        self.dataset = MOBIFACE(root_dir, subset)

        out_dir = 'MOBIFACE_'+subset
        self.result_dir = os.path.join(result_dir, out_dir)
        self.report_dir = os.path.join(report_dir, out_dir)
    ...
    def report(self, tracker_names, mytracker=None):
        assert isinstance(tracker_names, (list, tuple))
        if not mytracker:
            # assume tracker_names[0] is your tracker
            report_dir = os.path.join(self.report_dir, tracker_names[0])
            mytracker = tracker_names[0]
        else:
            report_dir = os.path.join(self.report_dir, mytracker)
        ...
        for name in tracker_names:
            ...
            for s, (img_files, anno) in enumerate(self.dataset):
                ...
                boxes[0] = anno[0]
                assert len(boxes) == len(anno), name + '\'s result on ' + seq_name + ' is wrong'

                ious, center_errors, norm_center_errors = self._calc_metrics(boxes, anno, width, height)
                ...
                performance[name]['seq_wise'].update({seq_name: {
                    ...
                    'norm_precision_curve': norm_prec_curve[s].tolist(),
                    'success_score': auc(np.linspace(0,1,self.nbins_iou), succ_curve[s]),#np.mean(succ_curve[s]),
                    'precision_score': prec_curve[s][20],
                    'norm_precision_score': auc(np.linspace(0,0.5, self.nbins_nce) , norm_prec_curve[s]),
            ...
            performance[name]['overall'].update({
                ...
                'norm_precision_curve': norm_prec_curve.tolist(),
                'success_score': succ_score,
                'precision_score': prec_score,
                'success_rate': succ_rate,
                'norm_precision_score': norm_prec_score,
                'speed_fps': avg_speed})
        ...
        self.plot_curves(tracker_names, mytracker)
    ...
    def make_video(self, tracker_names, seq_names=None, out_dir='visualisation',fps=15):
        if seq_names is None:
            seq_names = self.dataset.seq_names
        elif isinstance(seq_names, str):
            seq_names = [seq_names]
        ...
        for s, seq_name in enumerate(seq_names):
            out_fn = os.path.join(out_dir, seq_name+'.mp4')
            print('[%d/%d] Writing results on %s...' % (
                s + 1, len(seq_names), out_fn))
            ...
            fig.tight_layout()
            patches_list = []
            with writer.saving(fig, out_fn, 100):
                for f, img_file in tqdm(enumerate(img_files), total=len(img_files)):
                    ...
                    boxes = [anno[f]] + [
                        records[name][f] for name in tracker_names]
                    patches_list = draw_frame(image, ax.axes, patches_list, boxes,
                               legends=['GroundTruth'] + tracker_names,
                               colors=['w', 'r', 'g', 'b', 'c', 'm', 'y',
                                       'orange', 'purple', 'brown', 'pink'])
                    writer.grab_frame()
    ...
    def plot_curves(self, tracker_names, mytracker=None):
        # assume tracker_names[0] is your tracker
        if not mytracker:
            report_dir = os.path.join(self.report_dir, tracker_names[0])
        else:
        ...
        norm_prec_file = os.path.join(report_dir, 'normalised_precision_plots.png')
        key = 'overall'
        ...
        # title suffix
        if self.dataset.subset == 'all':
            title_suffix = 'all sequences'
        else:
            title_suffix = 'training sequences' if self.dataset.subset == 'train' else 'testing sequences'
        ...
        for i, name in enumerate(tracker_names):
            ...
            # legends.append('%s: [%.3f] - %.2f fps' % (name, performance[name][key]['success_score'], performance[name][key]['speed_fps']))
        ...
        ax.set(xlabel='Overlap threshold',
               ylabel='Success rate',
               xlim=(0, 1), ylim=(0, 1),
               title='Success plot on ' + title_suffix)
        ax.grid(True)
        fig.tight_layout()
        ...
        for i, name in enumerate(tracker_names):
            ...
            # legends.append('%s: [%.3f] - %.2f fps' % (name, performance[name][key]['precision_score'], performance[name][key]['speed_fps']))
        matplotlib.rcParams.update({'font.size': 7.4})
        legend = ax.legend(lines, legends, loc='center left',
                           bbox_to_anchor=(1, 0.5))
        ...
        ax.set(xlabel='Location error threshold',
               ...
               title='Precision plot on ' + title_suffix)
        ax.grid(True)
        fig.tight_layout()
        ...
        # sort trackers by normalised precision score
        tracker_names = list(performance.keys())
        prec = [t[key]['norm_precision_score'] for t in performance.values()]
        inds = np.argsort(prec)[::-1]
        tracker_names = [tracker_names[i] for i in inds]
        ...
        for i, name in enumerate(tracker_names):
            line, = ax.plot(thr_nce,
                            performance[name][key]['norm_precision_curve'],
                            markers[i % len(markers)])
            lines.append(line)
            legends.append('%s: [%.3f]' % (name, performance[name][key]['norm_precision_score']))
            # legends.append('%s: [%.3f] - %.2f fps' % (name, performance[name][key]['norm_precision_score'], performance[name][key]['speed_fps']))
        matplotlib.rcParams.update({'font.size': 7.4})
        legend = ax.legend(lines, legends, loc='center left',
                           bbox_to_anchor=(1, 0.5))
        
        matplotlib.rcParams.update({'font.size': 9})
        ax.set(xlabel='Normalised location error threshold',
               ylabel='Normalised precision',
               xlim=(0, thr_nce.max()), ylim=(0, 1),
               title='Normalsed precision plot on ' + title_suffix)
        ax.grid(True)
        fig.tight_layout()
        
        print('Saving normalised precision plots to', norm_prec_file)
